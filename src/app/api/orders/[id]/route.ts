...
export async function PUT(
  ...
  try {
    ...
    if (validatedData.estimatedTime) {
      updateData["deliveryInfo.estimatedTime"] = new Date(
        validatedData.estimatedTime,
      );
    }

    if (validatedData.actualDeliveryTime) {
      updateData["deliveryInfo.actualDeliveryTime"] = new Date(
        validatedData.actualDeliveryTime,
      );
    }
...
// PATCH /api/orders/[id] - Update order status (admin only) - alias for PUT
